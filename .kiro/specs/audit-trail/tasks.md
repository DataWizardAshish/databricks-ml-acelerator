...
- [x] 1. Create the `audit/` package with core data models and hashing
  - [x] 1.1 Create `audit/__init__.py` and `audit/models.py`
...
- [-] 2. Implement `AuditStore` — SQLite and Delta backends
  - [x] 2.1 Create `audit/store.py` with `AuditStore` Protocol and `SqliteAuditStore`
  ...
  - [x] 2.4 Implement `get_audit_store()` factory in `audit/store.py`
...
- [-] 3. Implement `AuditWriter` with fire-and-forget dispatch
  - [x] 3.1 Create `audit/writer.py` with `AuditWriter` class and `get_audit_writer()` singleton
...
- [-] 5. Implement `ChainVerifier`
  - [x] 5.1 Create `audit/verifier.py` with `ChainVerifier` class
...
- [-] 6. Add `run_id` to `AgentState` workspace dict and instrument agent nodes
  - [x] 6.1 Propagate `run_id` into the workspace dict in `agent/graph.py`
  ...
  - [x] 6.2 Instrument `rank_opportunities` in `agent/nodes.py`
  ...
  - [x] 6.3 Instrument `dry_run_explain` in `agent/trust_nodes.py`
    - After building updated state, emit `event_type="dry_run_planned"` with `payload={"dry_run_plan": plan}`
    - Wrap in `try/except Exception: pass`
    - _Requirements: 2.2_

  - [x] 6.4 Instrument `generate_code` in `agent/code_gen_nodes.py`
...
- [-] 7. Instrument FastAPI approval endpoints in `agent/graph.py`
  - [x] 7.1 Instrument `approve_opportunity()` in `agent/graph.py`
  ...
  - [x] 7.2 Instrument `confirm_dry_run()` in `agent/graph.py`
  ...
  - [x] 7.3 Instrument `approve_code()` in `agent/graph.py`
